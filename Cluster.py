...
import Exact
import random
ptk_name = "instance_artificial_01_26pol_hole"
ptk = "Instances/packing/"+ptk_name+".txt"
g = Graph.Graph(5,1,5)
g.initProblem(ptk)
...
for i in range(len(min_bet_graph_values)-1):
    fo += min_bet_graph_values[i][i+1]/5
print("Final FO value ",fo)
plt.title("Time Required: {:.2f}".format(fo))
plt.savefig("Images/"+ptk_name+".jpg",dpi=300)
plt.close()    
